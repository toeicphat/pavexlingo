...
const VocabularyTestScreen: React.FC<{
  ...
  const [isAudioChoiceSessionFinished, setIsAudioChoiceSessionFinished] =
    useState(false);
  ...
  const [selectedMatchingItem, setSelectedMatchingItem] =
    useState<MatchingItem | null>(null);
  const [matchedPairs, setMatchedPairs] = useState<string[]>([]);
  const [incorrectPairItems, setIncorrectPairItems] = useState<{
    item1: MatchingItem;
    item2: MatchingItem;
  } | null>(null);
  ...
  const [scramblerQuestions, setScramblerQuestions] = useState<
    ScramblerQuestion[]
  >([]);
  const [currentScramblerIndex, setCurrentScramblerIndex] = useState(0);
  const [scramblerScore, setScramblerScore] = useState(0);
  const [isScramblerSessionFinished, setIsScramblerSessionFinished] =
    useState(false);
  ...
  const [isSpellingSessionFinished, setIsSpellingSessionFinished] =
    useState(false);

  // State for audio dictation game
  const [audioDictationQuestions, setAudioDictationQuestions] = useState<
    AudioDictationQuestion[]
  >([]);
  const [currentAudioDictationIndex, setCurrentAudioDictationIndex] =
    useState(0);
  const [audioDictationScore, setAudioDictationScore] = useState(0);
  const [isAudioDictationSessionFinished, setIsAudioDictationSessionFinished] =
    useState(false);
  ...
  const [selectedDMatchWord, setSelectedDMatchWord] =
    useState<DMatchItemType | null>(null);
  const [correctDMatches, setCorrectDMatches] = useState<string[]>([]); // array of word strings
  const [incorrectDMatchPair, setIncorrectDMatchPair] = useState<
    string[] | null
  >(null); // [word, definition]
  ...
  const generateAudioChoiceQuestions = useCallback(() => {
    ...
    const questions = shuffledWords.map((correctItem: VocabItem) => {
      ...
      let pool = wordsForSession.filter(
        (w: VocabItem) => w.word !== correctItem.word,
      );
  ...
  const setupMatchingTurn = useCallback((deck: VocabItem[], turn: number) => {
    ...
    const turnItems = deck.slice(
      startIndex,
      startIndex + WORDS_PER_MATCHING_TURN,
    );
    ...
    const wordsForGrid = turnItems.map((item: VocabItem) => ({
      item,
      type: "word" as const,
    }));
    const defsForGrid = turnItems.map((item: VocabItem) => ({
      item,
  ...
  const handleShuffleScrambler = useCallback(() => {
    const unansweredQuestions = scramblerQuestions
      .slice(currentScramblerIndex)
  ...
  const setupDMatchTurn = useCallback((deck: VocabItem[], turn: number) => {
    ...
    const turnItems = deck.slice(
      startIndex,
      startIndex + WORDS_PER_DMATCH_TURN,
    );
    ...
    setDMatchWords(
      turnItems.map((item: VocabItem) => ({ item, type: "word" as const })),
    );
    setDMatchDefinitions(
      shuffleArray(turnItems).map((item: VocabItem) => ({
        item,
        type: "definition" as const,
      })),
    );
  ...
  const handleWordPractice = useCallback(
    (word: VocabItem, performance: "good" | "hard") => {
      const wordId = word.word.toLowerCase();
      updateWordSrsLevel(
        wordId,
        performance,
        word,
        `From '${testData.title}' list.`,
      );
    },
    [testData.title],
  );
  ...
  const handleAudioChoiceAnswer = (selectedItem: VocabItem) => {
    ...
    updatedQuestions[currentAudioChoiceIndex] = {
      ...currentQuestion,
      userAnswer: selectedItem.word,
    };
  ...
  const handleMatchingItemSelect = (clickedItem: MatchingItem) => {
    if (matchedPairs.includes(clickedItem.item.word) || incorrectPairItems)
      return;
    ...
    if (!selectedMatchingItem) {
      ...
      if (
        selectedMatchingItem.item.word === clickedItem.item.word &&
        selectedMatchingItem.type !== clickedItem.type
      ) {
        ...
        setIncorrectPairItems({
          item1: selectedMatchingItem,
          item2: clickedItem,
        });
        setSelectedMatchingItem(null);
        setTimeout(() => {
  ...
  const handleScramblerSubmit = (index: number) => {
    ...
    const isCorrect =
      ...
      question.original.word.toLowerCase();
  ...
  const handleDMatchDefinitionClick = (definition: DMatchItemType) => {
    ...
    if (selectedDMatchWord.item.word === definition.item.word) {
      ...
      setIncorrectDMatchPair([
        selectedDMatchWord.item.word,
  ...
  const activeModeClasses =
    "bg-blue-600 text-white shadow-xl transform scale-105 border-blue-700 ring-4 ring-blue-500/20";
  const inactiveModeClasses =
    "bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300 border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 hover:border-blue-400 dark:hover:border-blue-500 shadow-lg";
  ...
  const ModeButton: React.FC<{
    ...
    >
      <Icon
        className={`h-12 w-12 ${active ? "text-white" : "text-blue-500"}`}
      />
    ...
    return (
      <div>
        <div className="relative mb-6" style={{ perspective: "1000px" }}>
          ...
          >
            <div
              className="absolute w-full h-full bg-white rounded-xl flex items-center justify-center p-6 text-center"
              style={{
            ...
            >
              ...
              {currentWord.ipa && (
                <p className="text-xl text-slate-500 mt-2 font-mono">
                  {currentWord.ipa}
                </p>
              )}
            </div>
            <div
              className="absolute w-full h-full bg-slate-100 rounded-xl flex flex-col items-center justify-center p-6 text-center"
              style={{
                backfaceVisibility: "hidden",
                WebkitBackfaceVisibility: "hidden",
        ...
        <div className="grid grid-cols-3 gap-4 mb-4">
          <button
            onClick={() => {
              handleWordPractice(currentWord, "hard");
              handleNextCard();
            }}
            className="p-3 bg-red-500 text-white font-bold rounded-lg hover:bg-red-600 transition-colors"
          >
            Hard
          </button>
          <button
            onClick={() => {
              handleWordPractice(currentWord, "good");
              handleNextCard();
            }}
            className="p-3 bg-yellow-500 text-white font-bold rounded-lg hover:bg-yellow-600 transition-colors"
          >
            Good
          </button>
          <button
            onClick={handleNextCard}
            className="p-3 bg-green-500 text-white font-bold rounded-lg hover:bg-green-600 transition-colors"
          >
            Easy (No SRS)
          </button>
        </div>
    ...
    return (
      <div className="bg-white dark:bg-slate-800 p-6 md:p-8 rounded-xl shadow-lg border border-slate-200 dark:border-slate-700">
        <div className="text-center mb-8">
          <h3 className="text-2xl lg:text-3xl font-extrabold text-slate-800 dark:text-slate-100 mb-2">
            {title}
          </h3>
          <div className="inline-block px-6 py-4 bg-blue-50 dark:bg-slate-900 rounded-2xl border border-blue-100 dark:border-slate-600">
            <p className="text-sm text-slate-500 dark:text-slate-400 font-bold uppercase tracking-wider mb-1">
              Total Score
            </p>
          ...
          <div className="mb-8 bg-red-50 dark:bg-red-900/20 p-6 rounded-xl border border-red-100 dark:border-red-900/50">
            ...
            <div className="space-y-3 max-h-96 overflow-y-auto">
              {incorrectItems.map((item, idx) => (
                <div
                  key={idx}
                  className="bg-white dark:bg-slate-900 p-3 rounded-lg border border-red-200 dark:border-red-900/50 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-2"
                >
                  <div>
                    <span className="font-bold text-slate-800 dark:text-slate-200 text-lg mr-2">
                      {item.word.word}
                    </span>
                    {item.word.ipa && (
                  ...
                  <div className="text-sm">
                    ...
                    <span className="font-semibold text-red-600 dark:text-red-400 line-through mr-2">
        ...
        {incorrectItems.length === 0 && (
          <div className="mb-8 bg-green-50 dark:bg-green-900/20 p-6 rounded-xl border border-green-100 dark:border-green-900/50 text-center">
            <h4 className="text-lg font-bold text-green-800 dark:text-green-400 flex justify-center items-center gap-2">
              <CheckCircleIcon className="w-6 h-6" /> Perfect Score! Excellent
              job!
            </h4>
          </div>
        )}
    ...
    return (
      <div className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-lg border border-slate-200 dark:border-slate-700">
        <div className="flex justify-between items-center mb-6">
          ...
          <span className="font-semibold text-slate-500">
            {currentAudioChoiceIndex + 1} / {audioChoiceQuestions.length}
          </span>
        </div>
        <div className="mb-8">
          <AudioPlayer
            audioScript={currentQuestion.item.word}
            autoPlay={true}
          />
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
          {currentQuestion.options.map((option, idx) => {
            ...
            let buttonClasses =
              "flex flex-col items-center justify-center p-6 h-32 rounded-xl border-2 transition-all duration-200 font-semibold shadow-sm ";

            if (hasAnswered) {
              if (isCorrect) {
                buttonClasses +=
                  "bg-green-100 border-green-500 text-green-800 dark:bg-green-900/40 dark:border-green-600 dark:text-green-300";
              } else if (isSelected) {
                buttonClasses +=
                  "bg-red-100 border-red-500 text-red-800 dark:bg-red-900/40 dark:border-green-600 dark:text-green-300";
              } else {
                buttonClasses +=
                  "bg-slate-50 border-slate-100 text-slate-400 opacity-50 dark:bg-slate-800 dark:border-slate-700 dark:text-slate-600";
              }
            } else {
              buttonClasses +=
                "bg-white border-slate-200 hover:border-blue-400 hover:bg-blue-50 dark:bg-slate-700 dark:border-slate-600 dark:hover:bg-slate-600 dark:hover:border-blue-500 text-slate-800 dark:text-slate-100";
            }
            ...
            return (
              ...
              >
                ...
                {option.ipa && (
                  <span className="text-sm font-mono mb-1">{option.ipa}</span>
                )}
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 p-4 bg-slate-50 dark:bg-slate-900/50 rounded-xl border border-slate-200 dark:border-slate-700">
          <button
            onClick={() =>
              setCurrentAudioChoiceIndex((prev) => Math.max(0, prev - 1))
            }
          ...
          <label className="flex items-center cursor-pointer select-none">
            <div className="relative">
              ...
              />
              <div
                className={`w-12 h-6 rounded-full shadow-inner transition-colors ${isAutoAdvance ? "bg-blue-600" : "bg-slate-300 dark:bg-slate-600"}`}
              ></div>
              <div
                className={`absolute top-1 left-1 w-4 h-4 bg-white rounded-full shadow transition-transform ${isAutoAdvance ? "translate-x-6" : ""}`}
              ></div>
            </div>
          ...
          <button
            onClick={() =>
              setCurrentAudioChoiceIndex((prev) =>
                Math.min(audioChoiceQuestions.length - 1, prev + 1),
              )
            }
            disabled={
              currentAudioChoiceIndex === audioChoiceQuestions.length - 1
            }
    ...
    return (
      <div className="bg-white p-6 rounded-xl shadow-lg border border-slate-200">
        <h3 className="text-xl font-bold text-slate-800 text-center mb-4">
          Matching Game (Round {matchingTurn + 1} /{" "}
          {Math.ceil(fullMatchingDeck.length / WORDS_PER_MATCHING_TURN)})
        </h3>
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {matchingGridItems.map((gridItem, index) => {
            const isMatched = matchedPairs.includes(gridItem.item.word);
            const isIncorrect =
              incorrectPairItems?.item1 === gridItem ||
              incorrectPairItems?.item2 === gridItem;
            const isSelected = selectedMatchingItem === gridItem;

            let buttonClasses =
              "p-4 h-24 flex items-center justify-center text-center rounded-lg border-2 font-semibold transition-all duration-200 ";

            if (isMatched) {
              buttonClasses +=
                "bg-green-100 border-green-300 text-green-800 opacity-50 cursor-not-allowed";
            } else if (isIncorrect) {
              buttonClasses += "bg-red-100 border-red-500 text-red-800";
            } else if (isSelected) {
              buttonClasses +=
              ...
              buttonClasses +=
                "bg-white border-slate-300 hover:bg-blue-50 hover:border-blue-400 cursor-pointer";
            }
              ...
              >
                {gridItem.type === "word" ? (
                  <div className="flex flex-col items-center">
                    ...
                    {gridItem.item.ipa && (
                      <span className="text-xs mt-1 opacity-70 font-mono font-normal">
                        {gridItem.item.ipa}
                      </span>
                    )}
                  </div>
    ...
    return (
      <div className="bg-white p-6 rounded-xl shadow-lg border border-slate-200">
        <div className="flex justify-between items-center mb-4">
          ...
          <span className="font-semibold text-slate-500">
            {currentScramblerIndex + 1} / {scramblerQuestions.length}
          </span>
        </div>
        <div className="p-6 bg-slate-100 rounded-lg text-center">
          ...
          <p className="text-base text-slate-700 font-medium">
            "{currentQuestion.original.definition}"
          </p>
          {currentQuestion.original.ipa && (
        ...
        <div className="mt-6 text-center">
          <p className="text-4xl font-bold tracking-widest text-blue-600 my-4 p-4 bg-blue-50 rounded-lg">
            {currentQuestion.scrambled}
          </p>
          <form
            onSubmit={(e) => {
              e.preventDefault();
              handleScramblerSubmit(currentScramblerIndex);
            }}
          >
            <input
              type="text"
              ...
              onChange={(e) =>
                handleScramblerInputChange(
                  currentScramblerIndex,
                  e.target.value,
                )
              }
            ...
            {currentQuestion.isCorrect === false && (
              <div className="mt-2 text-center">
                <p className="text-green-600 font-semibold">
                  Correct answer: {currentQuestion.original.word}
                </p>
                {currentQuestion.original.ipa && (
                  <p className="text-sm font-mono text-green-700 opacity-80">
                    {currentQuestion.original.ipa}
                  </p>
                )}
              </div>
            ...
            {currentQuestion.isCorrect === null && (
              <div className="mt-4 flex gap-2 justify-center">
                <button
                  ...
                  className="px-4 py-2 bg-slate-200 rounded-md font-semibold text-slate-700 hover:bg-slate-300"
                >
                  Skip/Shuffle
                </button>
                <button
                  type="submit"
                  className="px-8 py-2 bg-green-600 text-white font-bold rounded-lg hover:bg-green-700"
                >
                  Check
                </button>
              </div>
  ...
  const renderSpellingRecall = () => {
    ...
    const revealedWord =
      currentQuestion.original.word.substring(
        0,
        currentQuestion.revealedCount,
      ) +
      "_".repeat(
        currentQuestion.original.word.length - currentQuestion.revealedCount,
      );
    ...
    return (
      <div className="bg-white p-6 rounded-xl shadow-lg border border-slate-200">
        ...
        <div className="mt-6 text-center">
          <p className="text-2xl font-bold tracking-[0.5em] text-blue-600 my-4 p-2 bg-blue-50 rounded-lg uppercase">
            {revealedWord}
          </p>
          <form
            onSubmit={(e) => {
              e.preventDefault();
              handleSpellingSubmit(currentSpellingIndex);
            }}
          >
            <input
              type="text"
              value={currentQuestion.userAnswer}
              onChange={(e) =>
                handleSpellingInputChange(currentSpellingIndex, e.target.value)
              }
              disabled={currentQuestion.isCorrect !== null}
              className={`w-full text-center text-lg p-3 border-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 transition-colors duration-200
                                ${currentQuestion.isCorrect === true ? "bg-green-100 border-green-500" : ""}
            ...
            {currentQuestion.isCorrect === null && (
              <div className="mt-4 flex gap-2 justify-center">
                <button
                  type="button"
                  onClick={() => revealLetter(currentSpellingIndex)}
                  className="px-4 py-2 bg-yellow-400 text-yellow-900 rounded-md font-semibold hover:bg-yellow-500"
                >
                  Hint (Reveal Letter)
                </button>
                <button
    ...
    return (
      <div className="bg-white p-6 rounded-xl shadow-lg border border-slate-200">
        <div className="flex justify-between items-center mb-4">
          ...
          <span className="font-semibold text-slate-500">
            {currentAudioDictationIndex + 1} / {audioDictationQuestions.length}
          </span>
        </div>
        ...
        {currentQuestion.meaningRevealed && (
          <div className="mt-4 p-4 bg-yellow-50 rounded-lg text-center flex flex-col justify-center items-center">
            <p className="text-sm text-yellow-700 font-semibold">
              Hint (Meaning): "{currentQuestion.original.definition}"
            </p>
            {currentQuestion.original.ipa && (
              <p className="text-xs font-mono text-yellow-700 opacity-80 mt-1">
                {currentQuestion.original.ipa}
              </p>
            )}
          </div>
        ...
        <div className="mt-6 text-center">
          <form
            onSubmit={(e) => {
              e.preventDefault();
              handleAudioDictationSubmit(currentAudioDictationIndex);
            }}
          >
            <input
              type="text"
              value={currentQuestion.userAnswer}
              onChange={(e) =>
                handleAudioDictationInputChange(
                  currentAudioDictationIndex,
                  e.target.value,
                )
              }
              disabled={currentQuestion.isCorrect !== null}
            ...
            {currentQuestion.isCorrect === null && (
              <div className="mt-4 flex gap-2 justify-center">
                <button
                  type="button"
                  onClick={() => revealMeaning(currentAudioDictationIndex)}
                  className="px-4 py-2 bg-yellow-400 text-yellow-900 rounded-md font-semibold hover:bg-yellow-500"
                >
                  Hint (Show Meaning)
                </button>
  ...
  const renderDefinitionMatch = () => {
    ...
    if (isDMatchTurnFinished) {
      return (
        <div className="bg-white p-8 rounded-xl shadow-lg border border-slate-200 text-center">
          <h3 className="text-2xl font-bold text-slate-800">Round Complete!</h3>
          <p className="text-lg text-slate-600 mt-2">
            You matched all words for this round.
          </p>
          <button
            onClick={handleDMatchNextTurn}
            className="mt-8 px-8 py-3 bg-blue-600 text-white font-bold rounded-lg hover:bg-blue-700 transition-colors"
          >
            Next Round
          </button>
    ...
    return (
      <div className="bg-white p-6 rounded-xl shadow-lg border border-slate-200">
        <h3 className="text-xl font-bold text-slate-800 text-center mb-4">
          Definition Match (Round {dMatchTurn + 1} /{" "}
          {Math.ceil(fullDMatchDeck.length / WORDS_PER_DMATCH_TURN)})
        </h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-3">
            {dMatchWords.map((wordItem, index) => {
              ...
              const isIncorrect =
                incorrectDMatchPair?.[0] === wordItem.item.word;

              let classes =
                "w-full text-left p-3 rounded-lg border-2 font-semibold text-sm transition-all duration-200 min-h-[60px] flex items-center ";
              if (isCorrect)
                classes +=
                  "bg-green-100 border-green-300 text-green-800 opacity-50 cursor-not-allowed";
              else if (isIncorrect)
                ...
                classes += "bg-blue-200 border-blue-500 ring-2 ring-blue-300";
              else
                classes +=
                  "bg-white border-slate-300 hover:bg-blue-50 hover:border-blue-400 cursor-pointer";
              ...
              return (
                <button
                  key={`word-${index}`}
                  onClick={() => handleDMatchWordClick(wordItem)}
                  className={classes}
                  disabled={isCorrect}
                >
                  <div className="flex flex-col w-full text-left">
                    ...
                    {wordItem.item.ipa && (
                      <span className="text-xs font-mono opacity-70 font-normal mt-0.5">
                        {wordItem.item.ipa}
                      </span>
                    )}
                  </div>
          ...
          <div className="space-y-3">
            {dMatchDefinitions.map((defItem, index) => {
              ...
              const isIncorrect =
                incorrectDMatchPair?.[1] === defItem.item.definition;

              let classes =
                "w-full text-left p-3 rounded-lg border-2 text-sm transition-all duration-200 min-h-[60px] flex items-center ";
              if (isCorrect)
                classes +=
                  "bg-green-100 border-green-300 text-green-800 opacity-50 cursor-not-allowed";
              else if (isIncorrect)
                classes += "bg-red-100 border-red-500 text-red-800";
              else if (selectedDMatchWord)
                classes +=
                  "bg-white border-slate-300 hover:bg-yellow-100 hover:border-yellow-400 cursor-pointer";
              else
              ...
              return (
                <button
                  key={`def-${index}`}
                  onClick={() => handleDMatchDefinitionClick(defItem)}
                  className={classes}
                  disabled={isCorrect || !selectedDMatchWord}
                >
      ...
      <div className="max-w-4xl mx-auto">
        <div className="bg-white p-6 rounded-xl shadow-lg border border-slate-200 mb-8">
          ...
          <p className="text-slate-500 text-center mt-2">
            {wordsForSession.length} words in this session
          </p>
        </div>
        ...
        <div className="mb-10">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            ...
            />
            <ModeButton
              active={mode === "audio_choice"}
              onClick={() => setMode("audio_choice")}
              icon={AudioChoiceIcon}
              label="Nghe & Chọn"
            />
            <ModeButton
              active={mode === "matching_game"}
              onClick={() => setMode("matching_game")}
              icon={GridIcon}
              label="Matching"
            />
            <ModeButton
            ...
            <ModeButton
              ...
              onClick={() => setMode("scrambler")}
              icon={ShuffleIcon}
              label="Scrambler"
            />
            <ModeButton
              active={mode === "spelling_recall"}
              onClick={() => setMode("spelling_recall")}
              icon={TypeIcon}
              label="Spelling"
            />
            <ModeButton
              active={mode === "audio_dictation"}
              onClick={() => setMode("audio_dictation")}
        ...
        {mode === "audio_choice" &&
          (isAudioChoiceSessionFinished
            ? renderAudioChoiceResults()
            : renderAudioChoice())}
        {mode === "matching_game" && renderMatchingGame()}
        {mode === "scrambler" &&
          (isScramblerSessionFinished
            ? renderScramblerResults()
            : renderScrambler())}
        {mode === "spelling_recall" &&
          (isSpellingSessionFinished
            ? renderSpellingResults()
            : renderSpellingRecall())}
