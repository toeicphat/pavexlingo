...
interface ListeningIntenseScreenProps {
  ...
  onSelectReflexTest: (testId: number) => void;
}

const ListeningIntenseScreen: React.FC<ListeningIntenseScreenProps> = ({
  currentUser,
  onBack,
  onSelectPart,
  onSelectReflexTest,
  ...
  const [selectedTest, setSelectedTest] =
    useState<ListeningIntenseTestData | null>(null);

  const [activeMacroMode, setActiveMacroMode] = useState<
    "selection" | "nghechep" | "phanxa"
  >("selection");
  ...
  const groupedTests = allListeningIntenseTests.reduce(
    (acc, test) => {
  ...
  return (
    <div className="container mx-auto px-4 py-12">
      <div className="max-w-6xl mx-auto">
        <button
          onClick={() => {
            if (activeMacroMode !== "selection") {
              setActiveMacroMode("selection");
            } else {
              onBack();
            }
          }}
        ...
        <div className="text-center mb-12">
          ...
          <p className="mt-4 text-lg text-slate-600 dark:text-slate-400">
            Nâng cao kỹ năng nghe với các bài luyện tập chuyên biệt dành cho mục
            tiêu điểm cao.
          </p>
        </div>

        {activeMacroMode === "selection" && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl mx-auto mt-12">
            <SelectionCard
              title="Luyện Nghe Chép chuyên sâu"
              description="Tuyển tập các bộ đề mới nhất (2024, 2026). Có chọn lọc cho các bạn mục tiêu điểm số cao."
              onClick={() => setActiveMacroMode("nghechep")}
              isComingSoon={false}
            />
            <SelectionCard
              title="Luyện phản xạ Part 2 (Beta)"
              description="Tăng tốc độ phản xạ với các dạng câu hỏi Part 2 thường gặp nhất."
              onClick={() => setActiveMacroMode("phanxa")}
              isComingSoon={false}
            />
          </div>
        )}

        {(activeMacroMode === "nghechep" || activeMacroMode === "phanxa") &&
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-6">
                  {groupedTests[group].map((test) => (
                    <SelectionCard
                      ...
                      title={test.title}
                      description={
                        activeMacroMode === "nghechep"
                          ? `Luyện nghe chuyên sâu ${test.title} ${group}.`
                          : `Luyện phản xạ Part 2 - ${test.title} ${group}.`
                      }
                      onClick={() =>
                        activeMacroMode === "nghechep"
